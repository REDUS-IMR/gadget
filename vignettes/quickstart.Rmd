...
As for now, the package is yet to find its way into CRAN. Therefore, you can
download and install from the pre-compiled binaries here:

```
https://github.com/REDUS-IMR/gadget/releases/latest
```

Or, you might want to compile it from the source in Github:
...
This guide demonstrates the gadgetr library ability of controlling a gadget
model run and extracting the relevant information from the run. 
...
As we can see in the previous section, the sample haddock model has a certain 
...
the metrics from both stocks and fleets objects at that particular step or
year.
